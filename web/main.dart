...
import 'dart:html';
import 'package:Exercice6/nav_menu.dart';
...
class ContactTable {
  ...
  ContactTable() {
    ...
    loadNotes = document.querySelector('#load-contacts');
    loadNotes.onClick.listen((e) {
      aContacts.fromJson(JSON.decode(window.localStorage['contacts']));
      aContacts.forEach((contact) => addRowData(contact.Name, contact.Email, contact.Phone));
    });
    saveNotes = document.querySelector('#save-contacts');
  ...
  addTableCaption(String title) {
    ...
    //contactTableCaption.style.backgroundColor='#3367D6';
    //contactTableCaption.style.color='#fefefe';
  ...
  addColumnTitle(row, String title, num width) {
    ...
    //columnHeader.style.backgroundColor='#3367D6';
    //columnHeader.style.color='#fefefe';
